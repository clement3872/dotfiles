...
# this script unmounts all ntfs and ext4 partitions on your system
...
for path,name in disk_info:
    os.system(f"sudo umount -t ntfs-3g {path} /media/{name}_mounted/")
    os.system(f"sudo rmdir /media/{name}_mounted/")

# mount ext4 partitions
disk_info = os.popen('sudo blkid | grep ext4').read().split('\n')[:-1]
disk_info = [(get_disk_path(disk), get_disk_name(disk)) for disk in disk_info]
for path,name in disk_info:
    os.system(f"sudo umount -t ext4 {path} /media/{name}_mounted/")
    os.system(f"sudo rmdir /media/{name}_mounted/")
